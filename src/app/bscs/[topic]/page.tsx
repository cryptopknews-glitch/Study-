...
import { getBscsTopicBySlug } from '@/lib/bscsTopics'
import MarkdownAnswer from '@/components/MarkdownAnswer'

export default function BscsTopicPage({ params }: { params: { topic: string } }) {
  const topic = getBscsTopicBySlug(params.topic)
  ...
  if (!topic) {
    return (
      <div className="px-4 py-8 space-y-3">
        ...
        <Link href="/bscs" className="text-primary text-sm font-medium">
          ← BSCS Preparation
        </Link>
      </div>
    )
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    try {
      const res = await fetch('/api/bscs', {
        method: 'POST',
  ...
  return (
    <div className="px-4 py-8 space-y-6 pb-16">
      <Link href="/bscs" className="text-sm text-primary font-medium">
        ← BSCS Preparation
      </Link>
